...
- `codex-app-server`
  ...
  - The current reference CLI fails closed and releases the claim instead of
    hanging or faking generation through the active Codex chat visual session.
...
```json
{"event":"codex_app_image_generate_unsupported","world_id":"stw_example","slot":"turn_cg:turn_0001","status":"released"}
```
...
- Visual jobs are never generated by the simulator binary, the active Codex chat
  visual session, or local drawing fallbacks. The reference CLI `codex-app-server`
  visual backend is reserved and fails closed by releasing the claim; packaged
  hosts must call their own image capability and complete the job explicitly.
