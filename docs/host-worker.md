...
- `codex-app-server`
  - The worker uses the official Codex app-server websocket, asks Codex App to
    create exactly one `imageGeneration` item, reads that item's `savedPath`,
    then completes the visual job by copying the PNG into the world store.
  - This is still Codex App image generation. The simulator does not call
    external image providers and does not draw placeholder assets.
  - Turn CG jobs from the active VN packet are claimable through the same queue
    and are prioritized before menu/stage/background jobs.
...
the embedding host to complete or release it. With
`--visual-backend codex-app-server`, the worker consumes the job through Codex
App image generation and completes it automatically.
...
Image job completed through Codex App app-server:

```json
{"event":"codex_app_image_generate_completed","world_id":"stw_example","slot":"turn_cg:turn_0001","status":"completed"}
```
...
- Visual jobs are never generated by the simulator binary or by local drawing
  fallbacks. The app-server backend delegates to Codex App `imageGeneration`
  and requires a real PNG `savedPath`.
