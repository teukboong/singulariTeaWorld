...
- `codex-app-server`
  ...
  - The desktop app's own embedded app-server is stdio-owned by the Electron
    host. The standalone app should start or receive an explicit websocket URL,
    for example `codex app-server --listen ws://127.0.0.1:<port>`.
...
- `codex-app-server` requires an explicit websocket URL. Missing URL emits
  `codex_app_server_dispatch_blocked`; it does not fall back to private desktop
  IPC.
