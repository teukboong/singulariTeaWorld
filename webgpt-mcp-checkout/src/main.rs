...
impl ServerHandler for WebGptMcpServer {
    ...
    #[expect(
        ...
        reason = "MCP dispatch keeps tool names and result serialization in one explicit table"
    )]
    async fn call_tool(
        &self,
        ...
        match request.name.as_ref() {
            ...
            "webgpt_turn_form" => {
                let params: WebGptTurnFormParams = tool::parse_json_object(arguments)?;
                let answer = tokio::task::spawn_blocking(move || run_turn_form(&params))
                    .await
                    .map_err(|error| {
                        McpError::internal_error(
                            format!("turn form task join failed: {error}"),
                            None,
                        )
                    })?
                    .map_err(|error| McpError::internal_error(error.to_string(), None))?;
...
fn validate_turn_form_spec(value: &Value) -> Result<()> {
    let Some(object) = value.as_object() else {
        bail!("turn_form_spec must be an object");
    };
    if object.get("schema_version").and_then(Value::as_str)
        != Some("singulari.webgpt_turn_form_spec.v1")
    {
        bail!("turn_form_spec schema_version mismatch");
    }
    for required in ["world_id", "turn_id", "choice_slots"] {
        if !object.contains_key(required) {
            bail!("turn_form_spec missing required field: {required}");
        }
    }
    Ok(())
}
    ...
    for line_result in reader.lines() {
        ...
        if value.get("event").is_none() {
            if let Ok(response) = serde_json::from_value::<WorkerRpcResponse>(value)
                && tx.send(WorkerMessage::Response(response)).is_err()
            {
                break;
            }
            continue;
    ...
    fn image_reference_paths_canonicalize_existing_images() {
        ...
        assert_eq!(
            ...
            vec![
                image
                    .canonicalize()
                    .expect("image fixture should canonicalize")
                    .display()
                    .to_string()
            ]
        );
